...
import (
	...
	grpc_generator "github.com/wso2-extensions/apim-gw-connectors/common-agent/pkg/k8s-resource-lib/pkg/generators/grpc"
)
...
func main() {
	...
	if endpoint, ok := endpoints[constants.ProductionType]; ok {
		httpRoute, err := gen.GenerateGRPCRoute(*apkConf, organization, gatewayConfig, *apkConf.Operations, &endpoint, constants.ProductionType, "unique-route-id", 1)
		if err != nil {
